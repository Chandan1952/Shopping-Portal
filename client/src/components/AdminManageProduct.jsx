...
const AdminManageProduct = () => {
  ...
  const fetchProducts = async () => {
    setLoading(true);
    try {
      const response = await fetch("https://shopping-portal-wptg.onrender.com/api/products");
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this product?")) {
      try {
        const response = await fetch(`https://shopping-portal-wptg.onrender.com/api/products/${id}`, { method: "DELETE" });
        if (response.ok) {
  ...
  const handleSave = async (product) => {
    try {
      const response = await fetch(`https://shopping-portal-wptg.onrender.com/api/products/${product._id}`, {
        method: "PUT",
                          ...
                          src={
                            product.image
                              ? product.image.startsWith("/uploads/")
                                ? `https://shopping-portal-wptg.onrender.com${product.image}`
                                : product.image
