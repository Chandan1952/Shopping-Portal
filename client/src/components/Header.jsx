...
export default function Header() {
  ...
  const handleLogout = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setUser(null);
        window.location.reload();  // ✅ Fix: Refresh page to clear state completely
      } else {
  ...
  const handleCategoryClick = (category) => navigate(`/products?category=${category.toLowerCase()}`);
    ...
    <>
      <header className="header">
        ...
        <div className="icons-container">
          ...
          <Link to="/cart-list" className="cart-link">
      ...
      <style>
        {`
          ...
          .nav-links span:hover { color: #ff3f6c; }

          .search-container { position: relative; width: 30%; }
